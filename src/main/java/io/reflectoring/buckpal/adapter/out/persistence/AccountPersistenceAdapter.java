...
class AccountPersistenceAdapter implements LoadAccountPort, UpdateAccountStatePort {
  ...
  public Account loadAccount(AccountId accountId, LocalDateTime baselineDate) {
    ...
    Long withdrawalBalance =
        activityRepository.getWithdrawalBalanceUntil(accountId.getValue(), baselineDate).orElse(0L);

    Long depositBalance =
        activityRepository.getDepositBalanceUntil(accountId.getValue(), baselineDate).orElse(0L);
